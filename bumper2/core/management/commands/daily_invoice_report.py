...
class Command(BaseCommand):
    ...
    help = 'Script:: DAILY_INVOICE_REPORT:: Daily Invoice report for previous day drops'

    def handle(self, *args, **options):
        current_time = timezone.now()
        logger.info("Script:: DAILY_INVOICE_REPORT:: Script started - current_time: %s" % current_time)
        ...
        try:
            ...
            cur.execute('''select
                ...
                CONVERT_TZ(bi.updated_at,'+00:00','+05:30') as invoice_updated_dt,
            ...
            fp = open('/tmp/today_invoice_report.csv', 'w')
            myFile = csv.writer(fp)
            myFile.writerow(field_names)
            myFile.writerows(rows)
            fp.close()
            fp = open('/tmp/today_invoice_report.csv','r')
            attachment = {
                'content': base64.b64encode(fp.read()),
                'name': 'Daily Invoice Report - ' + str(timezone.now().date()),
                'type': 'text/csv'
            }
            send_custom_notification('FINANCE_DAILY_INVOICE_REPORT', {'today_date':timezone.now().date()},
                                     attachments=[attachment])
        except:
            logger.exception("Script:: DAILY_INVOICE_REPORT:: Failed to get data for yesterday invoice. CHECK QUERY!!")
        logger.info("Script:: DAILY_INVOICE_REPORT:: Script ended")
